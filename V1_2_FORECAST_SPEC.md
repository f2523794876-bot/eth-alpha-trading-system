...
设计意图：15分钟预测更看重自身周期的执行结构（因子3权重22）和成交量/S-R距离（近端确认）；4小时预测更看重4H自身趋势（因子1权重22）和BTC对应周期联动（宏观相关性更重要，因子9权重12）；1小时预测介于两者之间。

### 4.3 防止重复计分的规则

- 因子12（三周期规则一致性代理）**不得**重新读取任何原始K线/EMA/Swing数据，只允许读取因子1/2/3已经算出的 `bull/bear/range` 主导方向作为输入，因为该规则一致性代理完全由三个因子各自的结论决定，不存在独立的原始数据来源。
