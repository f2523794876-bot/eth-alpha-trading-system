...
const base = {
  close: 100, closeToEma5: 0, trend4h: 'flat', trend1h: 'flat', volumeRatio20: 1.0,
  ...
  breakoutCount: null, breakdownCount: null, btcTrendState: 'flat', ethBtcRollingCorrelation: 0, logReturn1: 0
};
...
test('PO_RANGE_LOW_STRUCTURE：贴近支撑且trend4h∈{flat,down}', () => {
  const result = evaluatePoState({ ...base, distanceToSupportAtr: 0.1, trend4h: 'flat' });
  assert.equal(result.proxyState, 'PO_RANGE_LOW_STRUCTURE');
});

test('PO_STALL_HIGH_STRUCTURE：贴近压力且滞涨', () => {
  const result = evaluatePoState({ ...base, distanceToResistanceAtr: 0.1, distanceToSupportAtr: 5, trend4h: 'flat', closeToEma5: -0.5 });
  assert.equal(result.proxyState, 'PO_STALL_HIGH_STRUCTURE');
});

test('PO_RANGE_RECOVERY_STRUCTURE：区间内往复', () => {
  const result = evaluatePoState({ ...base, trend4h: 'flat', distanceToSupportAtr: 5, distanceToResistanceAtr: 5 });
...
test('auxiliaryEvidence隔离：funding/OI/taker等衍生品字段即使传入也不参与状态判定（不是函数已知参数，被忽略）', () => {
  const withDerivatives = { ...base, distanceToSupportAtr: 0.1, trend4h: 'flat', fundingRate: -0.05, fundingRateZScore: -5, openInterestChangeRatio: -0.9, longShortRatio: 0.1 };
  const without = { ...base, distanceToSupportAtr: 0.1, trend4h: 'flat' };
...
test('stateConfidence：completeness<0.8时扣20分，非HEALTHY时再扣15分，clamp[0,60]', () => {
  const healthy = evaluatePoState({ ...base, distanceToSupportAtr: 0.1, trend4h: 'flat', completeness: 0.95, qualityState: 'HEALTHY' });
  const degraded = evaluatePoState({ ...base, distanceToSupportAtr: 0.1, trend4h: 'flat', completeness: 0.5, qualityState: 'DEGRADED' });
