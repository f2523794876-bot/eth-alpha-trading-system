...
function validateVerifiedManifest(manifest,effective){
  ...
  if(manifest.datasetVersion!==effective.datasetVersion||manifest.symbol!==effective.symbol)throw fail('DATASET_MANIFEST_SCOPE_MISMATCH','Dataset manifest symbol/version does not match the request');
  const missing=required.filter(x=>!covered.has(`${x.symbol}:${x.marketType}:${x.interval}`));
  if(missing.length)throw fail('DATASET_MANIFEST_DEPENDENCY_UNGOVERNED','Dataset manifest does not govern every feature input dependency',{requiredDependencies:required,missingDependencies:missing,manifestDependencies:manifest.dependencies||[]});
