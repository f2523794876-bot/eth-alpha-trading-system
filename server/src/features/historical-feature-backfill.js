...
function validateVerifiedManifest(manifest,effective){const requiredIntervals=[...new Set(FEATURE_BAR_DEPENDENCIES.filter(x=>x.symbol===effective.symbol).map(x=>x.interval))];if(manifest.datasetVersion!==effective.datasetVersion||manifest.symbol!==effective.symbol)throw fail('DATASET_MANIFEST_SCOPE_MISMATCH','Dataset manifest symbol/version does not match the request');if(!requiredIntervals.every(interval=>manifest.intervals.includes(interval)))throw fail('DATASET_MANIFEST_INTERVAL_MISMATCH','Dataset manifest does not cover every required ETH interval',{requiredIntervals,manifestIntervals:manifest.intervals});if(manifest.dataTo<effective.toMs)throw fail('DATASET_MANIFEST_RANGE_INSUFFICIENT','Dataset manifest does not cover the requested range',{manifestDataTo:manifest.dataTo,requestedTo:effective.toMs});if(manifest.researchAvailabilityRuleVersion!==RESEARCH_AVAILABILITY_RULE_VERSION)throw fail('DATASET_RESEARCH_AVAILABILITY_VERSION_MISMATCH','Dataset manifest research-availability version does not match current code');}

function validateInputsWithinManifest(input,manifest,referenceTime){for(const dependency of FEATURE_BAR_DEPENDENCIES){for(const row of input[dependency.key]||[])if(row.openTime<manifest.dataFrom||row.closeTime>manifest.dataTo)throw fail('SOURCE_OUTSIDE_DATASET_MANIFEST',`${dependency.symbol} ${dependency.interval} source falls outside the verified manifest range`,{symbol:dependency.symbol,interval:dependency.interval,referenceTime,sourceOpenTime:row.openTime,sourceCloseTime:row.closeTime,manifestDataFrom:manifest.dataFrom,manifestDataTo:manifest.dataTo});}}
...
export async function runHistoricalFeatureBackfill({repository,options,executionTime=Date.now()}){
  ...
  const summary={runId,status:effective.dryRun?'DRY_RUN':'SUCCEEDED',dryRun:!!effective.dryRun,symbol:effective.symbol,interval:effective.interval,from:new Date(effective.fromMs).toISOString(),to:new Date(effective.toMs).toISOString(),featureVersion:effective.featureVersion,algorithmVersion:effective.algorithmVersion,datasetVersion:effective.datasetVersion,researchAvailabilityRuleVersion:RESEARCH_AVAILABILITY_RULE_VERSION,requestedPoints:allPoints.length,generatedPoints:0,alreadyPresent:0,blockedPoints:0,conflictPoints:0,failedPoints:0,earliestReferencePoint:allPoints.length?new Date(allPoints[0]).toISOString():null,latestReferencePoint:allPoints.length?new Date(allPoints.at(-1)).toISOString():null,sourceRange:{},asOfCheck:'PASS',results:[]};
  ...
  try{
    for(let offset=0;offset<allPoints.length;offset+=effective.batchSize){
      for(const referenceTime of allPoints.slice(offset,offset+effective.batchSize)){
        try{
          const input=await repository.loadHistoricalFeatureInputs({symbol:effective.symbol,targetBarCloseTime:referenceTime,historicalAsOfTime:referenceTime,replayNowMs:executionTime});
