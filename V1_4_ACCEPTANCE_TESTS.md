...
| v1.4c-production-lifecycle-1 | 2026-07-23 | 实施后生产验收附录：增加独立FeatureGenerator生命周期专项，覆盖新15m正式K线自动生成、24h/72h精确时间键消费、幂等、缺失阻断、租约互斥/过期接管、重启补漏、故障隔离、有界等待与SIGTERM优雅停止。本附录不改变冻结的T1–T30产品规范编号。 |

## V1.4C生产生命周期验收附录

| ID | 等级 | 场景 | 预期 |
|---|---|---|---|
| C-L1 | P0 | 新ETHUSDT 15m正式K线入库 | 独立FeatureGenerator自动生成同一`targetBarCloseTime`特征 |
| C-L2 | P0 | 特征完成后到达24h/72h预测边界 | 两个预测均只消费相同reference时间键的特征 |
| C-L3 | P0 | 同一时间键重复轮询/重启 | `feature_records`不重复写入 |
| C-L4 | P0 | 缺少正式K线或关键输入 | 不伪造特征，预测fail closed |
| C-L5 | P0 | 租约被其他实例持有 | 第二实例不启动、不并发写 |
| C-L6 | P0 | 原租约过期 | 新实例取得更高fencing token并接管 |
| C-L7 | P1 | 服务停机期间新增多根15m K线 | 重启按时间顺序有界批量补生成 |
| C-L8 | P1 | 单个特征时间键失败 | 记录阻断，collector不退出，后续周期继续 |
| C-L9 | P0 | 预测先于特征到达 | 有界等待精确特征；超时阻断，下一轮重试，不复用旧特征 |
| C-L10 | P0 | SIGTERM | 停止新任务、等待在途事务、释放租约后退出 |
