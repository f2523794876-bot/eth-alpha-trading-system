...
- 跨日通过同一`rollUtcDay`入口生成一次且仅一次`UTC_DAY_ROLLED`审计日志；重复tick和重复渲染不会重复记录，也不依赖页面重载。
...
- 成交价按`pricePrecision`四舍五入，数量按`quantityPrecision`向下取整；名义价值、手续费、滑点报告和盈亏在核心保持数值精度，UI只负责显示格式化，不回写核心数据。
...
- 暂停、风险锁定和数据阻断均禁止新开仓与加仓；已有仓位仍可使用新鲜且已收盘的15分钟K线执行保护。数据阻断且没有可信新收盘K线时不会用陈旧价格伪造离场。
...
- 独立的自然决策REST文件不覆盖`worthBetting`、`opportunityScores`、`signalPermission`、`triggerPlans`；无自然信号时以安全不交易作为正确结果。
