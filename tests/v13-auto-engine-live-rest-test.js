...
(async()=>{
  ...
  const price=raw.confirmedPrice,atr=raw.ltf15m.atr14||price*.01,riskDistance=Math.max(atr*4,price*.01),d={...raw,worthBetting:true,biasDirection:'long',dataHealth:'normal',isManual:false,opportunityScores:{...raw.opportunityScores,blocked:false,blockReasons:[]},signalPermission:{...raw.signalPermission,alignment:'full_aligned',level:'trend_entry_allowed',addOnAllowed:true},triggerPlans:{...raw.triggerPlans,long:{estimatedEntry:price,entryZone:[price-atr*.1,price+atr*.1],invalidation:price-riskDistance,targets:[price+riskDistance,price+2*riskDistance,price+3*riskDistance],riskReward:{status:'ok',grossValue:1,netValue:.9}}}},s=new X.Storage(),a=P.createPaperAccount();
