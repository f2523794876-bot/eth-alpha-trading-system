...
| 回填/manifest/replay/evaluation/report/audit模块 | 全部存在且互相编排（不是占位符）：`binance-kline-backfill.js`/`integrity-check.js`（回填+完整性）、`dataset-manifest-v2.js`/`multi-symbol-manifest-contract.js`/`canonical-manifest-content.js`（Manifest）、`replay-generator.js`/`replay-evaluator.js`（回放/评估，`cli-entry.js`调用）、`report-builder.js`/`purge.js`/`po-diagnostic.js`（报告）、`historical_validation.*`八张表（审计） | 直接读取各文件，见下文引用 |
...
| 失败状态/blocked reasons | `backfill_batches.status`∈{RUNNING,SUCCEEDED,FAILED,ATTENTION_REQUIRED}；`validation_runs.status`∈{RUNNING,SUCCEEDED,FAILED}，`blocked_reasons`结构化追加(phase/horizon/historicalAsOfTime/message/code) | `backfill-cli-entry.js`/`cli-entry.js` `markValidationRunFailed`（本轮原文读取） |
...
| 数据库/磁盘/内存/运行时间 | **部分可由代码确定，部分必须环境核验**——见下§三.12/§三.13 | 见下 |
...
**必须环境核验的部分**（代码本身无法确定，见§三.13）：目标PostgreSQL实例当前磁盘剩余空间、索引/TOAST/WAL实际膨胀系数、`historical_validation`schema当前已有数据量（是否已有此前测试遗留的行）、`pg.Pool`实际可用连接数上限（代码固定`max:10`，但服务器`max_connections`总量需环境查询）。
...
- **失败后恢复**：`backfill:market-bars --resume <backfill_batch_id>`（同symbol/interval/range/as-of严格匹配，见`assertResumeBatchCompatible`）；`ATTENTION_REQUIRED`批次**不可**`--resume`（人工介入前不得判定成功，见`V1_4D_DATA_BACKFILL_SPEC.md`§2.12）
- **是否产生数据库写入**：**是**（`market_bars`追加式INSERT；`backfill_batches`/`dataset_manifests`审计与内容寻址记录，二者均不可原地覆盖）

### Phase 5：历史回放

- **输入**：Phase 4产出的`dataset_version`
- **命令模板**：
  ```
  V14D_DATABASE_IDENTITY=research TEST_DATABASE_URL=<research_db_url> \
    ...
    --split 50/25/25
  ```
  （**注意**：本阶段`--from`是`researchFrom`，**不是**Phase 4的`backfillFrom`——见§C.2区分）
- **前置条件**：Phase 4的Manifest已`SUCCEEDED`；`validateReplayRange`要求执行时刻的真实UTC时间 ≥ `researchTo + 72h`
- **输出**：`validation_run_id`；`replay_generation_runs`/`replay_evaluation_runs`/`replay_snapshots`/`replay_outcome_events`记录
- **通过条件**：命令返回`plan`且未抛出异常，`validation_runs.status='SUCCEEDED'`
- **阻塞条件**：§4.1a manifest八步校验任一失败（数据集漂移等）；`RESUME_CHECKPOINT_INCONSISTENT`；生成/评估阶段未处理异常（`validation_runs.status`转为`FAILED`，`blocked_reasons`记录阶段/节奏点）
- **失败后恢复**：`--resume <validation_run_id>`（省略参数视为沿用原run，显式传入且不一致则`RESUME_PARAM_MISMATCH`拒绝；`weight-version`/`evaluation-version`每次仍须显式提供且须与原run已产生记录一致）；**已到达终态(SUCCEEDED/FAILED)的run禁止再次resume**
...
- 说明：`validation:walk-forward`内部对每个节奏点顺序执行"生成→评估"（见`cli-entry.js` `runWalkForward`循环体），不存在独立的"evaluation单独命令"——本阶段与Phase 5共享同一次调用与同一份产出。若因故只需要重跑评估侧（例如`evaluation-version`升级但`algorithm-version`/`dataset-version`不变），可用`--resume`挂载新`--evaluation-version`并配合`checkResumeVersionConsistency`的允许条件（该run名下尚无该evaluation-version记录时可采用）。
...
- **命令模板**：无固定命令；由**非执行者本人**的独立审查者，仅依据仓库代码+`historical_validation`schema只读查询，复现§G全部指标并核对§H偏差清单
- **前置条件**：Phase 7完成
...
## E. Go/No-Go 门禁（正式执行前，即Phase 4开始前，必须全部满足）
...
| 情形 | 处理规则 |
|---|---|
| checkpoint | `backfill_batches.last_completed_open_time`（回填侧）；`replay_generation_runs`/`replay_evaluation_runs`终态记录（回放侧，见`computeResumeCheckpoint`） |
| resume | `--resume <backfill_batch_id>`（严格同symbol/interval/range/as-of）；`--resume <validation_run_id>`（省略参数继承原run，显式且不一致则拒绝） |
| 幂等性 | `market_bars`：`ON CONFLICT(vintage_id) DO NOTHING`；`replay_snapshots`/`replay_outcome_events`：`ON CONFLICT DO NOTHING`；`dataset_manifests`：`ON CONFLICT(dataset_version) DO NOTHING`（同内容重复构建幂等返回既有行） |
...
| 何时必须从头开始 | Manifest构建`REJECTED`且原因是依赖数据本身有不可修复缺口（如Binance历史数据缺口无法补齐）；`RESUME_CHECKPOINT_INCONSISTENT`且无法通过既有审计记录判定安全恢复点；Phase 8发现数据污染且无法排除 |
...
| 数据完整性报告 | `dataset_manifests.integrity_check_result`（契约版本2为`per_interval_record_count`承载的`perDependencyIntegrityCheckResult`）；`backfill_batches`各批次记录 |
